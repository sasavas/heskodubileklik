...
let file = document.getElementById("hesqrcode");
...
form.addEventListener("submit", function (e) {
  const fileError = "QR kodunu içeren bir dosya seçmelisin.";
...
file.addEventListener('change', function(e){
  if(file.files.length > 0){
    normalify(file)
    // TODO: check if file contains a valid QR hes code
  }
})
...
email2.addEventListener("input", function (e) {
  if (
    email2.value.trim().length > 10 &&
    email2.value.trim().indexOf("@") > -1
  ) {
    normalify(email2);
  }
});
